...
-----END CERTIFICATE-----"#;
    ...
    fn parse_sink_cn() -> anyhow::Result<()> {
        let cn = "123.snk.piston.com";
        assert_eq!(cn_to_peer(cn)?, Peer::Sink(Sink::new("123.snk.piston.com")));
        Ok(())
    }

    #[test]
    fn parse_source_cn() -> anyhow::Result<()> {
        let cn = "321.src.piston.com";
        assert_eq!(
            cn_to_peer(cn)?,
            Peer::Source(Source::new("321.src.piston.com"))
        );
        Ok(())
    }
    ...
    fn reject_too_many_bits() {
        assert!(cn_to_peer("1233.source.blag.piston.com").is_err());
    }
}
