...
pub fn private_directory(path: &std::path::Path) -> Result<(), PlatformError> {
    ...
    unsafe {
        ...
        let result = CreateDirectoryW(p_u16_path, Some(&mut sa));
        match &result {
            Ok(_) => {
...
fn get_user_psid() -> anyhow::Result<Tow<HLOCAL, PSID>> {
    unsafe {
        ...
        match &result {
            Ok(_) => {
                return Err(anyhow!("initial GetTokenInformation() succeeded unexpectedly").into());
            }
            Err(err) => {
                // For some reason the first call always fails but fills in the desired size...
                if err.code() != TOO_SMALL {
                    let _ = result.context("initial GetTokenInformation() failed")?;
                }
            }
        };
